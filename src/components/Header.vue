<template>
  <div class="header_box">
    <div class="top">
      <router-link to="/index"><a><img src="@/assets/images/personal/logo.png"></a></router-link>
      <ul>
        ...
        <li v-if="this.$store.state.userToken">
          ...
          <router-link v-if="userType=='2'" to="/organizationIndex">
            <img :src=fileUrl(this.$store.state.userImg) style="width:30px; height:30px; border-radius: 50%; line-height: 2px;" v-if="this.$store.state.userImg&&this.$store.state.userImg!=''">
        ...
        <li class="serarch_input" :class="{searchInput:isShowInput}">
          <a @click="hidden"><img src="@/assets/images/personal/搜索.png"></a>
        </li>
        <li class="hidden_input"><input type="text" ref="input" v-model="searchVal" placeholder="请输入要查询的内容" @keyup.enter.native="hidden"></li>
      </ul>
    </div>
  </div>
</template>
