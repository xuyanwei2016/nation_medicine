    ...
    <div class='userother_content_bg'>
      ...
      <div class="medetails_wrap">
        ...
        <div class="medetails_top">
          <div class="mecontent_main_opt">
            <div class="mecontent_opt_list">
              <div class="noagreeH share_btn" style="position:relative;">
                <div style="display:inline-block"><span><img src="../../assets/images/home/share.png" alt=""></span>分享</div>
            ...
            <div class="mecontent_opt_list" @click="tapCollection"><!--detail.isCollection -->
              <div v-if="!isCollection" class="noagreeH"><span><img src="../../assets/images/home/noheart_icon.png" alt=""></span>收藏</div>
              <div v-if="isCollection" class="agreeH"><span><img src="../../assets/images/home/heart_icon.png" alt=""></span>取消收藏</div>
            </div>
          </div>
          ...
          <div class="mecontent_main_right">
            <div class="mecon_main_title">《{{detail.name}}》</div>
            <div class="mecon_main_rate">
              <el-rate v-model="value5" disabled  :colors="['#F66B20', '#F66B20', '#F66B20']"></el-rate>
              <div class="mecon_main_review"><span>{{commentTotal}}</span>人评论</div>
              <div class="mecon_main_read"><span>{{detail.browseNum}}</span>人阅读</div>
            </div>
            <div class="mecon_main_text">
              <div class="mecon_main_left">
                <div class="mecon_main_author">作者：
                  <span class="mecon_main_name" v-if="detail.authorId">
                    <router-link :to="{path:'/medicalother',query:{id:detail.authorId}}">{{detail.author}}</router-link>
                  </span>
                  <span class="mecon_main_name" v-if="!detail.authorId">{{detail.author}}</span>
                </div>
              ...
              <div class="mecon_main_right">
                <div class="mecon_main_author">译者：
                  <span class="mecon_main_name" v-if="detail.translatorId">
                    <router-link :to="{path:'/medicalother',query:{id:detail.translatorId}}">{{detail.translator }}</router-link></span>
                  <span class="mecon_main_name" v-if="!detail.translatorId">{{detail.translator}}</span>
                </div>
                <div class="mecon_main_isbn">出版社：<span>{{detail.publisher}}</span></div>
                <div class="mecon_main_isbn">版本：<span>{{detail.revision }}</span>版</div>
              </div>
            </div>
            <div class="mecon_main_price">
              <span class="mecon_main_money">{{detail.price?'￥'+detail.price:'免费'}}</span>
              <span
                class="mecon_main_dis">￥{{detail.pricing}}{{detail.discount?`(${detail.discount}折)`:''}}</span>
            </div>
          ...
          <div class="userother_content_left">
            ...
            <div class="medetails_content_cata" id="anchor-2">
              ...
              <div class="medetails_content_extends" style="margin-top: 0" v-if="detail.bookCatalog&&isShowOpen">
                <img class="medetails_content_imge" src="@/assets/images/home/extend.png">
                <span class="medetails_content_extend" @click="openBtn">展开</span>
              </div>

              <div class="medetails_content_extends" style="margin-top: 0" v-if="detail.bookCatalog&&!isShowOpen">
                <img class="medetails_content_imge" src="@/assets/images/home/extend.png" style="transform:rotate(180deg)">
          ...
          <div class="commentRight">
            <div class="subleft_bot1">
              ...
              <div class="meDetail_textbookList" v-for="(book, index2) in sameList" :key="index2" @mouseenter.prevent='mousemove(index2)'>
                ...
                <div class="meDetail_textbook"  v-if="isShowBook==index2" @mouseleave="mouseoutItem">
            ...
            <div class="mecom_left" v-if='detail.messageCode!=1'>
              <div class="subleft_botTitle">相关文章</div>
              <div class="subleft_botLine"></div>
              <div v-if="articleList.length>0&&articleList!=null">
                <div class="hot_list" v-for="(item,index) in articleList" :key="index">
                  <div class="hot_listImg"></div>
                  <div class="hot_listText"><router-link :to="{path:'articleread',query:{id:item.id}}" style="color:#666">{{item.synopsis}}</router-link></div>
                </div>
              </div>
              <div v-else style="margin-top:22px;text-align:center;color:rgb(102, 102, 102)">
                暂无数据
              </div>
            </div>
            <div class="medetails_bottom">
              <img class="medetails_active" src="@/assets/images/home/yizhuactive.png">
            </div>
          </div>
        </div>
      </div>
    ...
    methods: {
      ...
      getDetail() {
        ...
        ebookDetailAPI({
          ...
          console.log(JSON.stringify(this.detail.summary))
          // this.detail.summary ="<p class=\"content\">二，谨艰苦，或有裨于研究之方法，云尔。</p> \n<p class=\"content\">公元一九五六年丙申春惊蛰日，王邈达序于时中精舍，时年七十有九。</p>"
      ...
      goEbook(main) {
        ...
        if(main.bookid && main.markid) {
          this.$router.push({path: '/ebook', query: {bookid: main.bookid, markid: main.markid, messageCode: main.messageCode,name:main.bookName}})
        }
      },
      /*展开按钮*/
      openBtn(){
      ...
      readBook(item) {
        if(item.markid != null) {
          this.$router.push({path: '/ebook', query: {bookid: item.id, markid: item.markid, messageCode: item.messageCode}})
        }
      },
    }
