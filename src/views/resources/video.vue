  ...
  <div class="resContent">
    <div class="conRight">
      <div class="title">
        <h1>视频管理</h1>
        <ul>
          <li @click="$router.push('/addVideo')"><img src="../../assets/images/resources/ico-jia.png" width="14" alt="">添加视频</li>
      ...
      <div class="list">
        ...
        <el-table ref="multipleTable" :data="list" v-loading="dataLoading" element-loading-text="给我一点时间" highlight-current-row stripe @selection-change="chioceList" style="width: 926px;margin: 0 auto">
          ...
          <el-table-column label="时长" align="center"  width="100px">
            <template slot-scope="scope">
              <span>{{scope.row.timeLength}}</span>
              <!-- <span class="two-line-word" :title="scope.row.name">{{scope.row.name}}</span> -->
            </template>
          </el-table-column>
          <el-table-column label="文件大小" align="center"  width="110px">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" align="center" width="180px">
            <template slot-scope="scope" >
              <div class="operation">
                <i @click="$router.push({path:'/addVideo',query:{lookId:scope.row.id}})">查看</i><i v-if="scope.row.status==0||scope.row.status==3" @click="deleteOne(scope.row)">删除</i><i v-if="scope.row.status==3||scope.row.status==0" @click="$router.push({path:'/addVideo',query:{editId:scope.row.id}})">编辑</i><i v-if="scope.row.status==0" @click="oneStatus(scope.row)">发布</i>
              </div>
            </template>
          </el-table-column>
        </el-table>
...
<script>
  ...
  import {getVideoListAPI,delVideoAPI,updateVideoStatusAPI} from '../../api/resources';
...
状态：0:未提交 1：待审核 2：审核通过 3：审核不通过 4：已上架 5：已下架*/
      getList(){
        getVideoListAPI(this.listQuery).then(res=>{
        ...
        if(this.batchList.length == 0){
          ...
          if (dataType) {
            ...
            confirm.apply(this, ['确认删除吗？']).then(() => {
              delVideoAPI(JSON.stringify(ids)).then(res => {
      ...
      batchStatus(){
        if(this.batchList.length == 0){
          ...
          if (dataType) {
            const ids = getMultipleSelectionIDs(this.batchList);
            updateVideoStatusAPI(JSON.stringify(ids)).then(res => {
              this.isAlert=true;
              this.getList()
            })
          } else {
      ...
      oneStatus(item){
        updateVideoStatusAPI(JSON.stringify([item.id])).then(res => {
          this.isAlert=true;
          this.getList()
        })
      },
